    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack {
                ...
                Button { loadTerms() } label: { Image(systemName: "arrow.clockwise") }
            ...
            HStack {
                ...
                TextField("Search terms or aliases…", text: $searchText).textFieldStyle(.plain)
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("Add term").font(.headline)
                HStack(spacing: 8) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Correct phrase").font(.caption).foregroundColor(.secondary)
                        TextField("e.g. COMP9900", text: $newPhrase)
                            .textFieldStyle(.roundedBorder).frame(width: 150)
                    }
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Aliases (comma separated)").font(.caption).foregroundColor(.secondary)
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text(" ").font(.caption)
                        Button("Add") { addTerm() }
                            .buttonStyle(.borderedProminent).disabled(newPhrase.isEmpty)
                    }
                }
        ...
        backendClient?.listDictionaryTerms { items in
            let loaded = items.compactMap { d -> DictionaryEntry? in
                ...
                return DictionaryEntry(
                    ...
                    approved: d["approved"] as? Bool ?? true
                )
            }
    ...
    var body: some View {
        if isEditing {
            HStack(spacing: 8) {
                ...
                .buttonStyle(.borderedProminent).controlSize(.small).disabled(editPhrase.isEmpty)
            ...
            HStack(spacing: 10) {
                ...
                if entry.aliases.isEmpty {
                    Text("no aliases").font(.caption).foregroundColor(.secondary).italic()
                } else {
